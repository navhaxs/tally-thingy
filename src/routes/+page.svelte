            ...
            {#each tallies as tally, i (tally.id)}
                <li class="flex items-center gap-2 bg-white rounded shadow px-2 py-1 relative min-h-[56px]" animate:flip={{ duration: 200 }}>
                    ...
                    <div class="absolute right-0 top-0 h-full flex items-center">
                        ...
                        {#if pendingRemoveId === tally.id}
                            <div class="absolute left-0 top-full mt-2 bg-white border border-gray-300 rounded shadow px-4 py-2 z-10">
                                <p>Remove <strong>{tally.label}</strong>?</p>
                                <div class="flex gap-2 mt-2">
                                    <!-- Remove confirmation buttons -->
                                    <button class="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600 cursor-pointer" on:click={() => confirmRemove(tally.id)}>Yes</button>
                                    <button class="px-2 py-1 bg-gray-300 rounded hover:bg-gray-400 cursor-pointer" on:click={cancelRemove}>No</button>
                                </div>
                            </div>
                        {/if}
                    </div>
            ...
            {#if pendingReset}
                <div class="absolute left-0 top-full mt-2 bg-white border border-gray-300 rounded shadow px-4 py-2 z-10">
                    ...
                    <div class="flex gap-2 mt-2">
                        ...
                        <button class="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600 cursor-pointer" on:click={confirmReset}>Yes</button>
                        <button class="px-2 py-1 bg-gray-300 rounded hover:bg-gray-400 cursor-pointer" on:click={cancelReset}>No</button>
                    </div>
                </div>
            {/if}
        </div>
